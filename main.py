# =====================================================
# 全市场 Phase2 WebSocket asyncio 高精度监控
# =====================================================
...
SIGNAL_CSV = "signals_Promax_WS_async.csv"
...
async def phase_monitor(symbol):
    ...
    while True:
        ...
        async with lock:
            ...
            compression = "强" if (rng_now < 0.5*rng_hist) else "弱"
...
# ================== WebSocket订阅 ==================
async def subscribe_trade(symbol):
    global trade_queues
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            async with lock:
                                ...
                                price = float(data['p'])
                                qty = float(data['q'])
                                trade_queues[symbol].append({'price':price,'qty':qty,'quoteQty':price*qty,'time':int(time.time()*1000)})
                        elif msg.type == aiohttp.WSMsgType.ERROR:
                            break
        except Exception as e:
            print(f"[WebSocket异常重连] {symbol}", e)
        await asyncio.sleep(5)  # 异常重连间隔
...
async def main():
    ...
    tasks = []
    for s in symbols:
        tasks.append(asyncio.create_task(subscribe_trade(s)))
        tasks.append(asyncio.create_task(phase_monitor(s)))
    print(f"🚀 全市场 Phase2 WebSocket asyncio 高精度监控启动，监控币种数: {len(symbols)}")
